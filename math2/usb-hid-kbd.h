//---
// USB HID Keyboard interface (standalone version)
//
// This is a standalone implementation that doesn't require modifying
// gint's include directory. Just include this header in your project.
//---
...
/* USB HID keyboard interface - use with usb_open() */
...
/* Cancel check callback - return true to cancel sending */
typedef bool (*usb_hid_kbd_cancel_cb)(void);
...
/* usb_hid_kbd_type_string_cancellable(): Type a string with progress and cancellation
   
   Same as usb_hid_kbd_type_string_progress but also checks for cancellation.
   
   @str             Null-terminated string to type
   @progress_cb     Function called with (current_char, total_chars) after each char
   @cancel_cb       Function that returns true if sending should be cancelled
   @timeout_ticks   Timeout in 128Hz RTC ticks (0 = no timeout)
   Returns 0 on success, -1 on error, -2 on cancel, -3 on timeout */
...
/* usb_hid_kbd_press_timeout(): Press a key with timeout
   
   @modifiers      Modifier keys
   @key            Key to press
   @timeout_ticks  Timeout in 128Hz RTC ticks
